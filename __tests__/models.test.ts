...
describe('Google Generative AI Plugin', () => {
  beforeAll(async () => {
    // Initialize plugin
      ...
      if (googleGenAIPlugin.models && googleGenAIPlugin.models['TEXT_SMALL']) {
        try {
          ...
          if (typeof response === 'string') {
            ...
            // It's a TextStreamResult, handled differently in some core versions
            expect(response).toBeDefined();
          }
        } catch (error: any) {
          if (error.message?.includes('RESOURCE_EXHAUSTED')) {
            console.warn('Skipping test: Quota exhausted for TEXT_SMALL');
            return;
          }
          throw error;
        }
    ...
    test('should generate embeddings with TEXT_EMBEDDING model', async () => {
      ...
      if (googleGenAIPlugin.models && googleGenAIPlugin.models['TEXT_EMBEDDING']) {
        try {
          ...
          if (error.message?.includes('RESOURCE_EXHAUSTED')) {
            console.warn('Skipping test: Quota exhausted for TEXT_EMBEDDING');
            return;
          }
          throw error;
        }
    ...
    test('should generate JSON with OBJECT_SMALL model', async () => {
      ...
      if (googleGenAIPlugin.models && googleGenAIPlugin.models['OBJECT_SMALL']) {
        try {
          ...
          if (error.message?.includes('RESOURCE_EXHAUSTED')) {
            console.warn('Skipping test: Quota exhausted for OBJECT_SMALL');
            return;
          }
          throw error;
        }
    ...
    test('should generate JSON with OBJECT_LARGE model', async () => {
      ...
      if (googleGenAIPlugin.models && googleGenAIPlugin.models['OBJECT_LARGE']) {
        try {
          ...
          if (error.message?.includes('RESOURCE_EXHAUSTED')) {
            console.warn('Skipping test: Quota exhausted for OBJECT_LARGE');
            return;
          }
          throw error;
        }
    ...
    test('should describe an image with IMAGE_DESCRIPTION model', async () => {
      ...
      if (googleGenAIPlugin.models && googleGenAIPlugin.models['IMAGE_DESCRIPTION']) {
        try {
          ...
          if (error.message?.includes('RESOURCE_EXHAUSTED')) {
            console.warn('Skipping test: Quota exhausted for IMAGE_DESCRIPTION');
            return;
          }
          throw error;
        }
    ...
    test('should describe an image with custom prompt', async () => {
      ...
      if (googleGenAIPlugin.models && googleGenAIPlugin.models['IMAGE_DESCRIPTION']) {
        try {
          ...
          if (error.message?.includes('RESOURCE_EXHAUSTED')) {
            console.warn('Skipping test: Quota exhausted for custom IMAGE_DESCRIPTION');
            return;
          }
          throw error;
        }
